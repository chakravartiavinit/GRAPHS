    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            for (int j = 0; j < size; ++j)
            {
                int temp1;
                cin >> temp1;
                graph[temp].insert(temp1);
                graph1[temp].insert(temp1);
            }
            temp++;
        }
        int count = 0;
        vector<bool> prince(n + 1, false), princess(n + 1, false);
        for (int i = 1; i <= n; ++i)
        {
            if (graph1[i].size() > 0)
            {
                for (auto y : graph1[i])
                {
                    if (prince[y] == false)
                    {
                        ...
                        for (auto x : graph1[i])
                        {
                            if (x != alpha && graph1[x].find(alpha) != graph1[x].end())
                            {
                                graph1[x].erase(alpha);
                            }
                        }
                        princess[i] = true;
                        prince[alpha] = true;
        ...
        {
            int prince_not_chosen = -1, princess_not_chosen = -1;
            for (int i = 1; i <= n; ++i)
            {
                if (prince[i] == false && prince_not_chosen == -1)
                {
                    prince_not_chosen = i;
                }
                if (princess[i] == false && princess_not_chosen == -1)
                {
                    princess_not_chosen = i;
                }
            }
            if (prince_not_chosen == -1 || princess_not_chosen == -1)
            {
                cout << "OPTIMAL\n";
            }
            else
            {
                cout << "IMPROVE\n";
                cout << princess_not_chosen << " " << prince_not_chosen << "\n";
            }
        }
    }
}