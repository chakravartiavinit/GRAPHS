...
int main()
{
    ...
    while (t--)
    {
        int n;
        cin >> n;
        vector<int> order_of_visit(n);
        for (int i = 0; i < n; ++i)
        {
            cin >> order_of_visit[i];
        }
        int nodes_in_this_level = 1, level_till_now = 0;
        int index = 1;
        int sequence = order_of_visit[1];
        while (index < n)
        {
            int temp = 0;
            level_till_now++;
            for (int i = 0; i < nodes_in_this_level; ++i)
            {
                while (index < n)
                {
                    if (order_of_visit[index]>=sequence)
                    {
                        temp++;
                        sequence = max(sequence, order_of_visit[index]);
                        index++;
                    }
                    else
                    {
                        sequence = order_of_visit[index];
                        break;
                    }
                }
            }
            nodes_in_this_level = temp;
        }
        cout << level_till_now << "\n";
    }
}