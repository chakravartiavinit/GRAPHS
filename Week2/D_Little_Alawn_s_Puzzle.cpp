...
/*###############################################################################################################################################*/
void dfs(int start, map<int, vector<int>> &graph, vector<bool> &visited)
{
    visited[start] = true;
...
int main()
{
    ...
    while (t--)
    {
        int n;
        cin >> n;
        vector<vector<int>> vt(2, vector<int>(n));
        ...
        for (int i = 0; i < n; ++i)
        {
            graph[vt[0][i]].push_back(vt[1][i]);
            graph[vt[1][i]].push_back(vt[0][i]);
        }
        long long answer = 1;
